...
int main(int argc, char **argv) {
    ...
    tip_origin2<<0.0095, -0.057,  -0.1209117;
    ...
    double rand_theta;
    for (int j=0; j<10; j++){
    	rand_r = (rand()%40)/10000;
    	rand_theta = (rand()%6282)/1000;
    	rand_x = rand_r*cos(rand_theta);
    	rand_y = rand_r*sin(rand_theta);
	    
	        tgoal.trajectory = des_trajectory;
	        des_moving = des_gripper_affine2;
	        des_moving.translation()[0] += rand_x;
	        des_moving.translation()[1] += rand_y; 
	        ...
	        while (weight_data <= 2.5){
	            ...
	            ik_solver2.ik_solve(des_moving); //convert desired pose into equiv joint displacements
	            q_vec2 = ik_solver2.get_soln();
	            q_vec2(6) = gripper_ang2;
	            for (int i=0;i<7;i++) {
